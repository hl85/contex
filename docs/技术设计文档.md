...
为了兼顾开发效率与生产环境的隔离性，执行层采用了**可插拔的后端设计 (Pluggable Execution Backend)**。

*   **Mock 模式 (Development/Stage 1):**
    *   Sidecar 直接在宿主机通过 `subprocess` 启动 Python 脚本。
    *   优势：无需 Docker，调试方便，启动极快。
    *   劣势：无环境隔离，依赖宿主机环境。
    *   *当前 Stage 1 默认采用此模式。*

*   **Docker 模式 (Production):**
    *   Sidecar 通过 Docker API 管理 `contex-brain` 容器。
    *   优势：环境完全隔离，一致性强，安全可控。
    *   *推荐生产环境使用。*

采用 **"Minimal Core + Dynamic Extension" (最小核心 + 动态扩展)** 策略构建镜像，在确保核心能力开箱即用的同时，大幅降低初始包体体积（~400MB）。

* **基础环境 (Runtime Environment):**
* **OS:** Debian Slim (Bookworm)，极致精简。
* **Python 3.11+:** 提供高性能的异步计算与逻辑推理能力。支持 `pip install --user` 动态安装依赖。
* **Node.js LTS:** 提供 Web 服务支持与前端构建能力。
* **Process Manager:** 内置 **Supervisor**，作为 PID 1 进程守护 Code Server 与 Agent，确保服务自动重启与日志管理，防止僵尸进程。


* **生产力工具链 (Productivity Toolchain):**
* **Code Server:** 内置 VS Code 后端 (保留 bin/lib 核心)，支持代码编辑与调试。
* **Dev Tools:** 预置 **Git** (版本控制), **Curl/Httpie** (自测工具)。
* **Browser Engine:** 预置 **Playwright** (Core)，复用系统级 **Chromium**，避免重复下载数百兆的浏览器二进制文件。
* **Data Suite (On-Demand):** Pandas, NumPy, JupyterLab 等重型数据科学包采用**后置加载**策略，首次使用时自动通过 `pip install --user` 安装并持久化到挂载卷。
...
应用采用**自包含引导 (Self-Contained Bootstrap)** 与 **按需扩展 (On-Demand Expansion)** 结合的策略：

1. **环境检查：** 启动时检测 Docker 环境，若缺失则引导用户安装。
2. **最小镜像拉取：** 首次运行拉取极致精简的生产镜像 (Brain Container Lite, ~400MB)，实现快速启动。
3. **能力扩展：** 当用户首次触发需要重型工具（如数据分析）的任务时，容器自动在后台下载扩展包并持久化到挂载卷。
4. **无感升级：** 桌面端检测到新版本时，自动拉取新镜像并平滑重启容器，用户数据与已安装的扩展通过挂载卷保留。
